...
func NewRouter(d Deps) http.Handler {
	...
	auth.RegisterRoutes(mux, mw, d.Auth, d.Google, d.Config.Auth)
	tenant.RegisterRoutes(mux, mw, d.Tenants)
	oauthclient.RegisterRoutes(mux, mw, d.OAuthClient, d.RateLimiter)
	storage.RegisterRoutes(mux, mw, d.Storage, d.ObjectStore)
	event.RegisterRoutes(mux, mw, d.Events)
	jobqueue.RegisterRoutes(mux, mw, d.JobQueue)

	var handler http.Handler = mux
	handler = middleware.CORS(d.Config.CORS.AllowedOrigins)(handler)
